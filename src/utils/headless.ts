/**
 * Vendor of @headlessui/tailwindcss b/c it doesn't support ESM.
 * @module
 */
...
export default plugin.withOptions<Options>(({ prefix = "ui" } = {}) => {
  return ({ addVariant }) => {
    for (const state of ["open", "checked", "selected", "active", "disabled"]) {
      // TODO(tailwindlabs): Once `:has()` is properly supported, then we can switch to this version:
    ...
    addVariant(
      ...
      ":where([data-headlessui-focus-visible]) &:focus",
    );
    addVariant(
